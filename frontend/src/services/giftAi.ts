...
if (!API_BASE_URL || API_BASE_URL === 'undefined') {
  console.error('❌ GIFT_AI_URL is not configured properly!');
  console.error('   Current value:', API_BASE_URL);
  throw new Error('Gift AI service URL not configured');
}
  ...
  constructor() {
    ...
    this.api.interceptors.request.use(
      (config) => {
        ...
        // Debug logging
        console.log(`🚀 Gift AI Request: ${config.method?.toUpperCase()} ${config.baseURL}${config.url}`);
        
        return config;
      },
      (error) => {
        console.error('🚫 Gift AI Request interceptor error:', error);
    ...
    this.api.interceptors.response.use(
      (response) => {
        console.log(`✅ Gift AI Response: ${response.status} ${response.config.method?.toUpperCase()} ${response.config.url}`);
        return response;
      },
      (error) => {
        console.error(`❌ Gift AI Error: ${error.response?.status || 'Network'} ${error.config?.method?.toUpperCase()} ${error.config?.url}`, error.response?.data || error.message);
        return Promise.reject(error);
      }
    );
  }
