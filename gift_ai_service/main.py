...
# ========================================================================
# ORCHESTRATOR (Embedded with Lazy Init)
# ========================================================================
class GiftOrchestrator:
    ...
    async def ensure_initialized(self):
        """Lazy initialization - only connect on first use"""
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        vision_client = VisionAIClient()  # Fast - no connections
        
        logger.info("✅ Service ready (connections will be made on first request)")
        logger.info("⚡ Worker startup time: <5 seconds")
...
@app.post("/refresh_vector_store")
async def refresh_vector_store():
    """Refresh Vector Store (Lazy Init)"""
    orch = await get_orchestrator()
